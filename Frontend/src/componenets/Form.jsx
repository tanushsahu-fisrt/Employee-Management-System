...
const Form = () => {
  ...
  const[usercity,setusercity] = useState('gwalior');
  ...
  useEffect( () => { fetch("http://localhost:3000/getallbank")
                    .then( (res) => res.json() )
                    .then( (result) =>setbank(result.data) )
                    .catch( (err) => console.log('error' + err)) 
  },[])

  useEffect( () => { fetch("http://localhost:3000/getdesig")
                    .then( (res) => res.json() )
                    .then( (result) =>setdesignation(result.data) )
                    .catch( (err) => console.log('error' + err)) 
  },[])
  
  useEffect( () => { fetch("http://localhost:3000/getallcity")
                    .then( (res) => res.json() )
  ...
  const handleinsert = () => {
    axios.post("http://localhost:3000/insertanemp",{
      eno: id,
      ename: username,
      salary: usersalary,
      state: userstate,
  ...
  const handleupdate = () => {
    axios.post("http://localhost:3000/updateanemp",{
      eno: id,
      ename: username,
      salary: usersalary,
      state: userstate,
  ...
  const handledelete = () => {
    axios.post("http://localhost:3000/deleteanemp",{ eno: id, })
    .then( (data) => {
      ...
      <fieldset className="p-3 border border-black">
        ...
        <div className="flex-row mt-2 ">
          <div className="flex gap-5  leading-4 justify-center ">
            ...
            <select name="state" value={userstate} onChange={(e) => setuserstate(e.target.value)}>
            {
                state.map( (d) => {
                 return <option key={d.state}>{d.state}</option>
                })
              }
            </select>
            <br />
          ...
          <div className="flex gap-3  mt-2 leading-4 justify-center">
            ...
            <select name="city" value={usercity} onChange={(e) => setusercity(e.target.value)}>
            {
                state.map( (d) => {
                 return <option key={d.city}>{d.city}</option>
                })
              }
            </select>
            <br />
        ...
        <div className="flex  gap-5 mt-2">
          <div className="flex gap-5 mx-auto leading-4 ">
            ...
            <label htmlFor="bankCode">Bank Code :</label>
             <select name="bankCode" value={userbank} onChange={(e) => setuserbank(e.target.value)}>
              {
                bank.map( (d) => {
      ...
      <fieldset className="p-3 border border-black">
        ...
        <div className="flex  gap-5 mt-2 ">
          <div className="flex gap-5 mx-auto leading-4">
            ...
            <select name="department" value={userdepartment} onChange={(e) => setuserdepartment(e.target.value)}>
            ...
            <select name="designation" value={userdesignstion} onChange={(e) => setuserdesignstion(e.target.value)}>
              {
                designation.map( (d) => {
                  return <option key={d.designation}>{d.designation}</option>
                 })
              }
            </select>
            <br />
